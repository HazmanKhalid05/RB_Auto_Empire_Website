...
function renderCart() {
    const count = document.getElementById("cartCount"),
        box = document.getElementById("cartItems"),
    ...
    if (!cart.length) {
        box.innerHTML = '<p style="color:var(--text-dim);padding:18px 0;">Your cart is empty.</p>';
        return;
    }
...
function openQuick(product) {
    ...
    document.getElementById("quickReviews").textContent = (4.6 + Math.random() * 0.3).toFixed(1) + " (" + (80 + Math
        .floor(Math.random() * 250)) + " Reviews)";
...
function updateCompareBar() {
    const bar = document.getElementById("compareBar"),
        txt = document.getElementById("compareText");
    if (!bar || !txt) return;
...
function showCompare() {
    if (compareList.length < 2) {
        showToast("Choose 2 products first.");
        return;
    }
...
function productSearchSuggestions() {
    const input = document.getElementById("productSearch"),
        panel = document.getElementById("searchSuggestions");
    ...
    input.addEventListener("input", () => {
        ...
        panel.querySelectorAll("button").forEach(btn => btn.onclick = () => {
...
function enhanceProducts() {
    document.querySelectorAll("[data-category]").forEach((card, i) => {
        ...
        badge.className = "stock-badge " + (i % 3 === 0 ? "limited" : "in");
        badge.textContent = i % 3 === 0 ? "Limited Stock" : "In Stock";
        card.appendChild(badge);
        const q = document.createElement("button");
        q.type = "button";
        q.className = "quick-btn";
        q.textContent = "👁 Quick View";
        q.onclick = () => openQuick(productFromCard(card));
        card.appendChild(q);
        const rating = document.createElement("div");
        rating.className = "rating-mini";
        rating.textContent = "★★★★★ 4." + (7 + i % 3) + " • " + (90 + i * 17) + " reviews";
        ...
        if (row) {
            ...
            add.className = "btn btn-outline small-btn";
            add.type = "button";
            add.textContent = "Add Cart";
            add.onclick = () => addToCart(productFromCard(card));
            row.after(add);
            const cmp = document.createElement("label");
            cmp.className = "compare-check";
            cmp.innerHTML = '<input type="checkbox"> Compare';
            row.after(cmp);
            cmp.querySelector("input").addEventListener("change", e => {
                ...
                if (e.target.checked) {
                    if (compareList.length >= 2) {
                        ...
                        showToast("Only 2 products can be compared.");
                        return;
                    }
...
function initVehicleFinder() {
    ...
    btn.addEventListener("click", () => {
        ...
        if (!brand || !model) {
            showToast("Choose brand and model first.");
            return;
        }
        document.querySelectorAll("[data-category]").forEach((card, i) => {
...
function checkoutFromCart() {
    ...
    const subtotal = cart.reduce((s, i) => s + i.price * i.qty, 0);
    activeItem = cart.length + " cart item(s)";
    activePrice = subtotal;
    activeQty = 1;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    initVehicleFinder();
    initFaq();
    document.getElementById("cartFloating")?.addEventListener("click", () => document.getElementById(
        "cartDrawer")?.classList.add("active"));
    document.getElementById("closeCart")?.addEventListener("click", () => document.getElementById("cartDrawer")
        ?.classList.remove("active"));
    document.getElementById("quickClose")?.addEventListener("click", closeQuick);
    document.getElementById("quickAddCart")?.addEventListener("click", () => quickProduct && addToCart(
        quickProduct));
    document.getElementById("quickBuyNow")?.addEventListener("click", () => {
        if (quickProduct) {
            closeQuick();
            openCheckout(quickProduct.title, quickProduct.price);
        }
    });
    ...
    document.getElementById("clearCompare")?.addEventListener("click", () => {
        compareList = [];
        document.querySelectorAll(".compare-check input").forEach(i => i.checked = false);
        updateCompareBar();
    });
    document.getElementById("compareClose")?.addEventListener("click", () => document.getElementById(
...
processPayment = function(e) {
    ...
    const form = document.getElementById("billingForm");
    successBox.innerHTML =
        `<span class="big-check">✅</span><strong>Payment Successful!</strong><br>Thank you, <b>${customer}</b>.<div class="invoice-box"><b>Invoice:</b> ${orderNo}<br><b>Item:</b> ${activeQty} × ${activeItem}<br><b>Total:</b> RM ${total}<br><b>Payment:</b> ${payment}<br><b>Estimated Delivery:</b> 2–5 working days</div>`;
    successBox.style.display = "block";
    form.style.display = "none";
    showToast("Order confirmed! Invoice generated.", "success");
    if (activeItem.includes("cart item")) {
        cart = [];
        saveCart();
    }
};
...
/* ===== Premium homepage hero video controls ===== */
(function initPremiumHeroVideo() {
    const setup = () => {
        const video = document.getElementById("heroVideo");
        const soundButton = document.getElementById("heroSoundToggle");
        const playButton = document.getElementById("heroPlayToggle");
        if (!video || !soundButton || !playButton) return;
        ...
        const updateSoundUI = () => {
            ...
            soundButton.setAttribute("aria-label", enabled ? "Turn video sound off" :
            "Turn video sound on");
            soundButton.querySelector(".sound-icon").textContent = enabled ? "🔊" : "🔇";
            soundButton.querySelector(".control-label").textContent = enabled ? "Sound On" : "Sound Off";
        };

        const updatePlayUI = () => {
            const paused = video.paused;
            playButton.setAttribute("aria-pressed", String(paused));
            playButton.setAttribute("aria-label", paused ? "Play background video" :
                "Pause background video");
            playButton.querySelector(".control-icon").textContent = paused ? "▶" : "Ⅱ";
        ...
        soundButton.addEventListener("click", async () => {
            try {
                ...
                showToast("Tap play first, then enable sound.");
            }
        });
...
/* ======================================================
   CONTACT PAGE — GOOGLE MAP BUSINESS CARD
   ====================================================== */
document.addEventListener("DOMContentLoaded", () => {
    const copyAddressBtn = document.getElementById("copyAddressBtn");
    const businessAddress =
        "No. 45, Jalan Automotif 3/1, Kawasan Perindustrian Seksyen 15, 40200 Shah Alam, Selangor, Malaysia.";
    ...
    copyAddressBtn.addEventListener("click", async () => {
        try {
            ...
            showToast("Warehouse address copied to clipboard.", "success");
        } catch (error) {
            const temporaryInput = document.createElement("textarea");
            temporaryInput.value = businessAddress;
