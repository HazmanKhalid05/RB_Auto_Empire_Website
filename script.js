...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll(".grid-cards, .org-grid, .stats-grid")
    .forEach((group) => {
      Array.from(group.children).forEach((child, i) => {
        child.style.setProperty(
          "--reveal-delay",
          Math.min(i * 0.09, 0.54) + "s",
        );
      });
    });
  ...
  document
    .querySelectorAll("section, .card, .org-card, .stat-item")
    .forEach((item) => {
...
function refreshCheckoutSummary() {
  ...
  document.getElementById("summaryItemPrice").innerText =
    "RM " + (activePrice * activeQty).toFixed(2);
...
function renderCart() {
  ...
  if (!cart.length) {
    box.innerHTML =
      '<p style="color:var(--text-dim);padding:18px 0;">Your cart is empty.</p>';
    return;
  }
  box.innerHTML = cart
    .map(
...
function openQuick(product) {
  ...
  document.getElementById("quickReviews").textContent =
    (4.6 + Math.random() * 0.3).toFixed(1) +
    " (" +
    (80 + Math.floor(Math.random() * 250)) +
    " Reviews)";
...
function updateCompareBar() {
  ...
  if (!bar || !txt) return;
  txt.textContent = compareList.length
    ? compareList.map((p) => p.title).join(" vs ")
...
function productSearchSuggestions() {
  ...
  const titles = [...document.querySelectorAll("[data-category] h3")].map((x) =>
    x.textContent.trim(),
  );
...
function enhanceProducts() {
  document.querySelectorAll("[data-category]").forEach((card, i) => {
    ...
    rating.textContent =
      "★★★★★ 4." + (7 + (i % 3)) + " • " + (90 + i * 17) + " reviews";
    card
      .querySelector(".card-info")
      ?.insertBefore(rating, card.querySelector(".card-info p"));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("quickAddCart")
    ?.addEventListener("click", () => quickProduct && addToCart(quickProduct));
  ...
  const setup = () => {
    ...
    const updateSoundUI = () => {
      ...
      soundButton.setAttribute(
        "aria-label",
        enabled ? "Turn video sound off" : "Turn video sound on",
      );
      soundButton.querySelector(".sound-icon").textContent = enabled
        ? "🔊"
        : "🔇";
      soundButton.querySelector(".control-label").textContent = enabled
        ? "Sound On"
        : "Sound Off";
    };
    ...
    const updatePlayUI = () => {
      ...
      playButton.querySelector(".control-icon").textContent = paused
        ? "▶"
        : "Ⅱ";
